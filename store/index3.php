...
<div id="wrapper">
	<div id="note">
		...
		Welcome to <?=$VendorID?>'s great store
	</div>
	<div id="content">
		<div id="productlist">
			...
			<?php
			require "connect.php";
			$result = $mysqli->query("SELECT * FROM oui_deliver_shop WHERE Vendor = $VendorID");
			while($row=mysqli_fetch_assoc($result))
			{
				echo '<a rel="facebox" href="orderpage.php?id='.$row['Id'].'&trnasnum='.$transnum.'"><p style="color: red;">'.$row['Name'].'</p></a><br>';
			}
		...
		<div id="orderlist">
			...
			/*	require "connect.php";*/
				$result3 = $mysqli->query("SELECT * FROM orders WHERE confirmation='$transnum'");
			...
			/*	$result3 = mysql_query("SELECT * FROM orders WHERE confirmation='$transnum'");*/
				...
				<tr>
				  ...
				  <td><div align="center">
				  <?php
				  $result5 = $mysqli->query("SELECT sum(total) FROM orders WHERE confirmation='$transnum'");
			...
			<?php
				...
				require "connect.php";
				  ...
				  $result5 = $mysqli->query("SELECT sum(total) as total FROM orders WHERE confirmation='$transnum'");
					while($row5 = mysqli_fetch_array($result5))
					  { 
						echo $row5['total']; //COME BACK HERE
					  }
				  ?>
